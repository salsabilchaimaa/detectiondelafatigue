...
===========================================================================
 Modèle   : DDD_weights.h5 / DDD_model_augmented.keras (EfficientNetB0, 4 classes)
...
# ─── Supporte les deux architectures : ancienne (128) et nouvelle (256+128+BN)
# ═══════════════════════════════════════════════════════════════════════════════

def build_model_architecture(num_classes=4, img_size=100):
    """Reconstruit l'architecture améliorée (Phase 2 fine-tuning).
    Utilise la Functional API pour correspondre exactement au modèle entraîné
    dans train_ddd_model3.py.
    """
...
def load_weights_manual(model, h5_path):
    """Charge les poids depuis un fichier .h5 manuellement.

    Contourne un bug de Keras 2.11 où model.load_weights() échoue avec
    'ValueError: axes don't match array' dû à un groupe H5 supplémentaire
    ('top_level_model_weights') qui perturbe le parseur interne.
    """
    import h5py
    with h5py.File(h5_path, 'r') as f:
        for layer in model.layers:
            ...
            for w in layer.weights:
                wname = w.name
                if wname in f:
                    layer_weights.append(np.array(f[wname]))
                else:
                    found = False
                    for gname in f:
                        grp = f[gname]
                        if isinstance(grp, h5py.Group) and wname in grp:
                            layer_weights.append(np.array(grp[wname]))
                            found = True
                            break
                    if not found:
                        print(f"[WARN] Poids non trouvés : {wname}")
                        return False
            if layer_weights:
...
if os.path.exists(WEIGHTS_PATH) and os.path.getsize(WEIGHTS_PATH) > 100_000:
    # Priorité aux poids (reconstruction architecture locale)
    model = build_model_architecture()
    ok = load_weights_manual(model, WEIGHTS_PATH)
    if ok:
        print(f"[INFO] Poids charges depuis : {WEIGHTS_PATH}")
    else:
        print(f"[WARN] Chargement manuel partiel, fallback sur .keras")

elif os.path.exists(_KERAS_PATH) and os.path.getsize(_KERAS_PATH) > 100_000:
    model = load_model(_KERAS_PATH)
    print(f"[INFO] Modele complet charge : {_KERAS_PATH}")

elif os.path.exists(_H5_PATH) and os.path.getsize(_H5_PATH) > 100_000:
    model = load_model(_H5_PATH)
    print(f"[INFO] Modele complet charge : {_H5_PATH}")

else:
    print(f"[ERROR] Aucun modele trouve dans {OUTPUT_DIR}")
    print(f"        Lancez d'abord : python train_ddd_model.py")
    exit(1)

print(f"[INFO] Modele pret — entree : {model.input_shape}")
...
while True:
    ...
    if yawn_roi is not None:
        ...
        cv2.rectangle(frame, (mx, my), (mx + mw, my + mh), (200, 0, 255), 2)
